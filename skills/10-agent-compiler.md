...
> **거울 → 대리인 교차점(telos 경계).** 14개 팩(= 당신의 *증거 기반 자기지도*)은 컴파일 *전에* 이미
> 프로젝트의 일차 산출물로 존재합니다 — 읽고 따져보는 *거울*. 이 스킬은 그 지도를 *행동하는*
> `AssistantProfile`로 물질화하는 단계, 곧 **거울에서 대리인으로 건너가는 지점**입니다
> ([README — 거울이냐 대리인이냐](../README.md#거울이냐-대리인이냐--자기명시화가-먼저다)). 그래서 활성화는
> 자동이 아니라 shadow-validation·사람 검토를 거칩니다: 당신의 패턴을 *당신 이름으로 실행*하는 순간
> 편향 증폭·책임 귀속 같은 *인간적 결과*가 들어오기 때문입니다("becoming you"는 검토 요하는 적용주장,
> [01 §7.1](../spec/01-kernel-schema.md)). 자기명시화(거울)에는 이 부담이 없습니다.
...
- **출처:** `user.identity_roles`(주), `user.persona_core`(안정 가치/우선순위) = **페르소나**;
  `user.memory_project_graph` = **현재 프로젝트 맥락(피연산자)**.
- **담는 것:** 에이전트가 *누구로서*(페르소나) 이 작업을 하는가 + *어떤 프로젝트 맥락 위에서*(피연산자).
- **⚠️ 경계 — 맥락은 페르소나가 아니다(전이성).** `memory_project_graph` 슬라이스는 이 섹션에
...
- **왜:** 같은 작업도 "리뷰어로서"인지 "저자로서"인지(페르소나), 그리고 *어느 프로젝트의*(맥락)에 따라
  달라집니다. **이 섹션의 페르소나 부분이 비면 안 됩니다** — identity_roles 누락이 바로 이 공백이며(§2),
  그 경우 컴파일러는 `coverage` 갭을 강하게 로깅하고 기본 역할로만 동작합니다.
