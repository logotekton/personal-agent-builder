...
> **텔로스 주의(거울이 먼저).** 이 스킬들이 만드는 일차 산출물은 *행동하는 에이전트*가 아니라 14개
> 팩 = **당신의 증거 기반 자기지도**입니다("나도 모르는 나"의 명시화). 마지막 단계 [S10 컴파일러](./10-agent-compiler.md)가
> 그 지도를 *행동하는* `AssistantProfile`로 컴파일하는데, 그건 **거울 → 대리인** 교차점이라 사람
> 검토를 요하는 별개 단계입니다([README — 거울이냐 대리인이냐](../README.md#거울이냐-대리인이냐--자기명시화가-먼저다)).
> 스킬 01–09·11은 그 *교차 이전*의 추출·정련 메커니즘이라 텔로스와 무관하게 동일합니다.
...
입력/출력 계약과 품질 게이트(G1–G6)로 강제됩니다. 확장 스킬 15(암묵지 채굴)·16(ingest 판정
게이트)은 **새 단계를 만들지 않습니다** — 15는 `mine_or_ask` 고도의 심화, 16은 07(레코드 게이트)의
런-레벨 보완입니다. 어휘는 [커널 스키마](../spec/01-kernel-schema.md),
...
> 규칙: 스킬 문서는 [Canonical Design Contract(v0.3)]를 따릅니다. 정식 이름(핵심 13개 + 확장 2개 스킬,
