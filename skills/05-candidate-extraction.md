...
- [ ] **행동 언어(G4)** — `concise_claim`이 *관찰된 행동*만 기술하는가. "싫어한다/때문이다" 같은
  추측 심리·동기를 단정하지 않았는가.
- [ ] **전이성 테스트 — 주체이지 주제인가([S02 §1.1](./02-session-mining.md#11-전이성-테스트--주체를-캐고-주제를-캐지-마라-mine-the-decider-not-the-topic))** —
  *프로젝트를 바꿔도 참인* 당신의 패턴(결정·추론·선호·휴리스틱)인가, 아니면 *이 프로젝트에서만 참인
  사실*인가. **프로젝트 사실은 페르소나·결정·암묵지·스타일·산출물·위험·워크플로 후보 타입으로 만들 수
  없다** — 오직 `ProjectMemoryCandidate`(→`memory_project_graph`) 또는 (여러 프로젝트에 걸친 지속
  도메인 지식이면) `DomainOverlayCandidate`(→`domain_overlays`)로만 적재한다. 전이 가능한 패턴이면
  프로젝트 고유 디테일을 벗기고 `concise_claim`을 *패턴*으로 쓰고 `scope`도 프로젝트명이 아닌 *패턴
  조건*으로 단다. (리트머스: "같은 프로젝트를 *다른 사람*이 해도 똑같이 말할 내용이면 → 프로젝트 사실".)
