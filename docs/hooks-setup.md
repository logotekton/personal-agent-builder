...
> `.codex/config.toml` (Codex CLI). Both call one shared entrypoint, [`tools/pab`](../tools/pab),
> which is a **STUB** today (it logs intent and exits 0; it does not stage to OpenCrab yet).
> Make `pab` call OpenCrab MCP to go live. The full per-host adapter map is
> [skills/14-host-binding.md](../skills/14-host-binding.md).
...
[`tools/pab`](../tools/pab)는 **자리표시 스텁**입니다 — 무엇을 *할지* 로그로 남기고 `exit 0`만
합니다. **아직 OpenCrab에 아무것도 스테이징하지 않습니다.** 그래서 지금 훅을 켜도 동작은
안전무해하지만 라이브 포착은 아닙니다. 로그는 `${PAB_LOG:-$TMPDIR/pab-hooks.log}`에 쌓입니다.

**라이브로 만들려면** `pab`의 각 `:` 자리(case 분기)를 실제 호출로 교체:
