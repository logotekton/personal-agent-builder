...
> convergence report. **5 of the 14 instance packs are seeded so far** (plus the evaluation
> cases — 6 packs by the convergence count) — an early **L0→L1** state whose single highest-
> leverage next move the convergence report names exactly. Spec ground truth:
...
정식 팩 이름을 **키로 하는 단일 YAML 매핑**입니다. 현재 **14개 중 5개 인스턴스 팩이 시드**(+
평가케이스 팩 = 수렴 기준 6팩)되어 있고, 각 레코드는 동일 이름의 [`user.*` 스키마](../../schemas)와
...
| 6 | `user.tacit_heuristics` | ✅ 1건 | 암묵 판단 규칙 | `heuristic` | [schema](../../schemas/user.tacit_heuristics.schema.json) |
...
| 4, 7–12, 14 | `artifact_policy`, `red_flags`, `workflow_playbooks`, `domain_overlays`, `tool_stack`, `boundary_authority`, `memory_project_graph`, `drift_history` | ⬜ 미시드 | (아직 증거 없음) | — | [schemas](../../schemas) |

> 8개 팩이 아직 비어 있는 것은 **버그가 아니라 상태**입니다. 이것이 정확히 *coverage를 올리는
> 다음 단계*이며, 증거가 없는 팩을 가짜로 채우지 않는 것이 이 프로젝트의 G1 원칙입니다. 현재
> 위치(**L0 Seed**, L1까지 팩 하나·L2를 정조준)와 *그 다음 한 수*는 아래
> [수렴 리포트](#4-convergence-reportmd--측정된-수렴)가 숫자로 보여줍니다.
...
위 인스턴스 집합과 `evaluation-cases.yaml`을 입력으로,
...
(L0–L4)를 계산한 결과입니다. "에이전트가 점점 나처럼 된다"가 느낌이 아니라 **숫자**라는 점을
실제 데이터로 보여줍니다. 같은 산출은 [`../../tools/`](../../tools)의 검증·수렴 스크립트나
OpenCrab의 `opencrab_pack_qa`로 재현할 수 있습니다.

## 읽는 순서 (추천)

1. **[`instance-records.yaml`](./instance-records.yaml)** — 시드된 5개 팩의 확인된 레코드.
...
3. **[`runtime-adapter.md`](./runtime-adapter.md)** — 위 레코드들이 하나의 통제된 프로필로
   합쳐진 모습.
4. **[`convergence-report.md`](./convergence-report.md)** — 이 모든 게 얼마나 수렴했는가, 숫자로.
