...
> convergence report. **7 of the 14 instance packs are seeded** (plus the evaluation cases —
> 8 packs by the convergence count) at maturity **L2 Working**, after `revolution-01` turned the
> data-engine wheel once on real records (merge actuator + the eval.004 correction →
> `decision_fidelity` 0.75→0.92, `merge_rate` NA→0.059): see
> [`revolution-01/README.md`](./revolution-01/README.md). Spec ground truth:
...
정식 팩 이름을 **키로 하는 단일 YAML 매핑**입니다. 현재 **14개 중 7개 인스턴스 팩이 시드**(+
평가케이스 팩 = 수렴 기준 8팩)되어 있고(revolution-01에서 `boundary_authority`·`drift_history`가
추가됨), 각 레코드는 동일 이름의 [`user.*` 스키마](../../schemas)와
[통합 베이스 레코드](../../schemas/record.base.schema.json)를 동시에 만족합니다
([팩 카탈로그](../../spec/03-pack-catalog.md)와 1:1 대응).
...
| 6 | `user.tacit_heuristics` | ✅ 1건 (revolution-01에서 **병합**: `repetition_count`=2) | 암묵 판단 규칙 | `heuristic` | [schema](../../schemas/user.tacit_heuristics.schema.json) |
...
| 13 | `user.evaluation_cases` | ✅ 6건 (→ 별도 파일 `evaluation-cases.yaml`) | 충실도 테스트 케이스 | `evalcase` | [schema](../../schemas/user.evaluation_cases.schema.json) |
| 14 | `user.drift_history` | ✅ 1건 (→ 별도 파일 `drift-history.yaml`; 교정 `DriftRecord`) | 변경·대체 이력 | `drift` | [schema](../../schemas/user.drift_history.schema.json) |
| 4, 7–10, 12 | `artifact_policy`, `red_flags`, `workflow_playbooks`, `domain_overlays`, `tool_stack`, `memory_project_graph` | ⬜ 미시드 | (아직 증거 없음) | — | [schemas](../../schemas) |

> 6개 팩이 아직 비어 있는 것은 **버그가 아니라 상태**입니다. 증거가 없는 팩을 가짜로 채우지 않는
> 것이 이 프로젝트의 G1 원칙입니다. 현재 위치(**L2 Working** — revolution-01에서 데이터-엔진을
> 한 바퀴 돌려 L0→L2 도약)와 *그 다음 한 수*는 아래
> [수렴 리포트](#4-convergence-reportmd--측정된-수렴)와
> [`revolution-01/README.md`](./revolution-01/README.md)가 숫자로 보여줍니다.
...
1. **[`instance-records.yaml`](./instance-records.yaml)** — 시드된 7개 팩의 확인된 레코드.
   `user.identity_roles`의 창립자 귀속 레코드가 다른 모든 팩의 기준점입니다. 가장 흥미로운
   암묵지는 `user.tacit_heuristics`(교정·diff에서 나온 "이건 이렇게" 규칙)입니다.
