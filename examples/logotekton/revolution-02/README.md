...
> 수정되어, **라이브 티어는 L0 Seed**이고 이 회전은 **L0 → L0 유지**입니다(**콘텐츠 깊이 vertical 0** —
> 유일한 ≥3 팩이 *메타* eval, 정직한 위치). `decision_fidelity`·`merge_rate`·`coverage`(시드폭 0.57→0.71) 등 *다른 숫자는 모두
...
# (3) T2 측정 — 콘텐츠-깊이 게이트 적용 후 L0 Seed(콘텐츠 깊이 0); df 1.00 / coverage 엄격 0.07(시드폭 0.71) / drift 0.89
python tools/convergence_report.py examples/logotekton    # L0 Seed; df 1.00; coverage 0.07(시드폭 0.71)
