...
> 속성-검증된 견고함을 가졌다는 강한 긍정 신호. 위반 2건 중 1건만 실제 적용 대상.
- **[Fixed] (LOW) correction_cost 부동소수 합 순서 의존.** `sum(corr_vals)/len` 의 비결합 합산이 평가
  케이스 순서에 따라 마지막 ULP(~5.55e-17)를 바꿔 직렬화 JSON 의 byte-동일성(결정성 논제)을 깸 —
  `math.fsum` 으로 순서무관 정확합산. 게이트 임계(≤0.3/≤0.15)와는 16자릿수 차이라 티어 불변, 잠금
  correction_cost 0.0833 도 불변(단일값 1/12).
- **[Not applied] 병합 non-fixpoint 후보** — 같은 canonical_key 레코드 2개의 재병합이 트윈을 붕괴시킨다는
  퍼저 보고는 *이 트리에서 재현되지 않음*: 현재 `plan_batch`+`_project_candidate`(it.1 intra-batch dedup)가
  두 후보를 `[novel, duplicate]` → 1 레코드로 올바르게 정리하고 멱등(merged==merged2)임을 확인. (퍼저가
  intra-batch 투영이 없는 변형을 본 것으로 판단 — 코드 변경 없음.)
- **[Added] 회귀 테스트 1건**(`TestPropertyDeterminismIt23`). 총 164→165.
