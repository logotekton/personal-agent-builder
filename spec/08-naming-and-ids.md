...
> **OpenCrab 배치.** 이 4개 클래스는 OpenCrab에서 *세 운영 프로젝트*로 배치합니다 — 방법+형태
> (`skill.pab.*` + `user.*`)는 재사용 가능한 **빌더** 프로젝트에, 세션 증거·pending 후보 보드는
> **증거 아카이브** 프로젝트에, 승인된 데이터+런타임(`personal.<subject>.*` + `*.runtime_adapter`)은
> 주인당 하나인 **개인 에이전트** 프로젝트에 둡니다. 빌더로 *후보를 만들고*, 증거 아카이브에
> *검토 전 상태를 보존하고*, 확정된 것만 개인 에이전트로 *인제스트*합니다
> ([07 프로젝트 토폴로지](./07-opencrab-9space-crosswalk.md#프로젝트-토폴로지--빌더-장치-증거-아카이브-개인-에이전트-3-project)).
